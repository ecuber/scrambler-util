...
var mathlib = (function() {
    ...
    _.search = function(state, minl, MAXL) {
        ...
        if (!this.inited) {
            ...
            for (var i = 0; i < this.N_STATES; i++) {
                var stateParam = this.state_params[i];
    ...
    _.toStr = function(sol, moveMap, powerMap) {
        var ret = [];
        for (var i = 0; i < sol.length; i++)
            ret.push(moveMap[sol[i][0]] + powerMap[sol[i][1]]);
    ...
    _.idaSearch = function(state, maxl, lm) {
        ...
        for (var move = 0; move < this.N_MOVES; move++) {
            ...
            var curState = state.slice();
            for (var power = 0; power < this.N_POWER; power++) {
                for (var i = 0; i < N_STATES; i++)
                    curState[i] = this.move[i][move][curState[i]];

                if (this.idaSearch(curState, maxl - 1, move)) {
