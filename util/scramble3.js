// FROM CSTIMER, MODIFIED BY THE SCRAMBLER TEAM
...
const scramble3 = (function(getNPerm, get8Perm, setNPerm, set8Perm, getNParity, getPruning, Cnk, fact, rn, rndEl) {
        ...
        for (i = 0; i < 2187; ++i)
            if ((occ[i >> 5] & 1 << (i & 31)) == 0) {
                ...
                for (s = 0; s < 16; s += 2) {
                    cornConjugate(c, s, d);
                    idx = $getTwist(d);
                    idx == i && (SymStateTwist[count] |= 1 << (s >> 1));
                    occ[idx >> 5] |= 1 << (idx & 31);
    ...
    function $initPhase2(obj) {
        ...
        for (depth2 = obj.maxDep2 - 1; depth2 >= prun; --depth2) {
            ...
            if (obj.preIdx != 0) {
                ...
                let pow;
                if (axisPre == axisLast) {
                    pow = (preMove[obj.preIdx] % 3 + obj.move[obj.sol - 1] % 3 + 1) % 4;
                    obj.move[obj.sol - 1] = axisPre * 3 + pow;
                } else if (depth2 > 1 && axisPre % 3 == axisLast % 3 && ~~(obj.move[obj.sol - 2] / 3) == axisPre) {
                    pow = (preMove[obj.preIdx] % 3 + obj.move[obj.sol - 2] % 3 + 1) % 4;
    ...
    function searchVerify(obj, facelets) {
        var count = 0, i;
        for (i = 0; i < 54; ++i) {
    ...
    function countU(b) {
        for (var c = 0, a = 0; a < b.length; a++) b[a] == -1 && c++;
        return c;
    }
    ...
    function getAnyScramble(_ep, _eo, _cp, _co, _rndapp, _rndpre) {
        ...
        do {
            ...
            var co = _co.slice();
            var cp = _cp.slice();
            var neo = fixOri(eo, countU(eo), 2);
            var nco = fixOri(co, countU(co), 3);
            var nep, ncp;
            var ue = countU(ep);
            var uc = countU(cp);
            if (ue == 0 && uc == 0) {
